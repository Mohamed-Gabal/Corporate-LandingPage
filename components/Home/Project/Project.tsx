...
const Project = () => { 
  return (
    <div className='pt-16 pb-16 bg-black'>
      ...
      <SectionHeading subTitle="Portfolio" title="Transforming ideas into" title2="digital reality" />
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-20 w-[80%] mx-auto gap-6'>
        {Projects_Images.map((image, index) => (
          <div key={image}>
            <Tilt> 
              <Image src={image} alt={`Project image ${index + 1}`} width={450} height={550} className='rounded-md' />
            </Tilt> 
          </div>
        ))}
      </div>
